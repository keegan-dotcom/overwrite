  ...
  return (
    <div className="mb-2 border-2 border-line bg-pane">
      <div className="flex flex-wrap items-center gap-x-4 gap-y-1.5 px-3 py-2">
        <span className="flex items-center gap-2 font-mono text-[13px] font-bold uppercase tracking-[0.1em]">
        ...
        <span className="font-mono text-[13px] font-bold uppercase tracking-[0.04em] text-paper">
          {strategyLabel(st.config)}
        </span>
        <span className="h-3.5 w-px bg-line" />

        {/* what's actually resting/held right now */}
        <span className="min-w-0 flex-1 truncate font-mono text-[13px] text-fog">
          {orders.length > 0
            ? orders.map((o) =>
        ...
        {(st.premium_recent ?? 0) > 0 && (
          <span className="font-mono text-[13px] text-mint">premium {fmtUsd(st.premium_recent ?? 0)}</span>
        )}

        {/* controls */}
        <div className="flex items-center gap-1.5">
          {unwinding && (
            <span className="border-2 border-amber px-2.5 py-1 font-mono text-[13px] font-bold uppercase text-amber">
              closing…
            </span>
          )}
          {pos.length > 0 && !unwinding && (
            <button onClick={() => act("unwind", () => hostedUnwind(deriveWallet, ownerEoa),
              ...
              disabled={!!busy}
              className="border-2 border-paper bg-amber px-2.5 py-1 font-mono text-[13px] font-bold uppercase text-ink shadow-hardsm transition-transform hover:-translate-y-px disabled:opacity-50">
              {busy === "unwind" ? "…" : "Unwind ✕"}
            </button>
          )}
          {!killed && (
            live ? (
              <button onClick={() => act("pause", () => hostedSetLive(deriveWallet, ownerEoa, false))}
                disabled={!!busy}
                className="border-2 border-amber px-2.5 py-1 font-mono text-[13px] font-bold uppercase text-amber transition-colors hover:bg-amber hover:text-ink disabled:opacity-50">
                {busy === "pause" ? "…" : "Pause"}
              </button>
            ) : (
              <button onClick={() => act("live", () => hostedSetLive(deriveWallet, ownerEoa, true),
                ...
                disabled={!!busy}
                className="border-2 border-paper bg-accent px-2.5 py-1 font-mono text-[13px] font-bold uppercase text-ink shadow-hardsm transition-transform hover:-translate-y-px disabled:opacity-50">
                {busy === "live" ? "…" : "Go live →"}
              </button>
            )
          )}
          {killed ? (
            <button onClick={() => act("resume", () => hostedPause(deriveWallet, ownerEoa, false))}
              disabled={!!busy}
              className="border-2 border-mint px-2.5 py-1 font-mono text-[13px] font-bold uppercase text-mint transition-colors hover:bg-mint hover:text-ink disabled:opacity-50">
              {busy === "resume" ? "…" : "Un-kill"}
            </button>
          ) : (
            <button onClick={() => act("kill", () => hostedPause(deriveWallet, ownerEoa, true),
              ...
              disabled={!!busy}
              className="border-2 border-rose px-2.5 py-1 font-mono text-[13px] font-bold uppercase text-rose transition-colors hover:bg-rose hover:text-ink disabled:opacity-50">
              {busy === "kill" ? "…" : "Kill"}
            </button>
          )}
        </div>
      ...
      {(killed || !live) && !unwinding && pos.length > 0 && (
        <div className="flex flex-wrap items-center gap-x-2 gap-y-1 border-t-2 border-amber bg-amber/10 px-3 py-1.5">
          <span className="font-mono text-[13px] font-bold uppercase tracking-[0.06em] text-amber">⚠ You are NOT flat</span>
          <span className="min-w-0 flex-1 font-mono text-[13px] text-amber">
      ...
      {err && <div className="border-t border-rose px-3 py-1 font-mono text-[13px] text-rose">{err}</div>}
    </div>
  );
}
