  ...
  return (
    <div className="border-2 border-line bg-pane">
      <div className="flex flex-wrap items-center justify-between gap-3 px-4 py-3">
        <div>
          <div className="font-mono text-[13px] uppercase tracking-[0.14em] text-mint">
        ...
        <div className="flex flex-wrap gap-2">
          <button
            ...
            className={`border-2 px-3.5 py-1.5 font-mono text-[14px] font-bold uppercase shadow-hardsm transition-transform hover:-translate-x-px hover:-translate-y-px ${
          ...
          <button
            ...
            className="border-2 border-line px-3.5 py-1.5 font-mono text-[14px] uppercase text-fog transition-colors hover:border-fog hover:text-paper"
          >
            ⬇ Download config
          </button>
          <button
      ...
      {open === "claude" && (
        <div className="space-y-3 border-t-2 border-line px-4 py-3">
          ...
          <div>
            <div className="mb-1 font-mono text-[13px] text-fog">Claude Code (one line):</div>
            <pre className="overflow-x-auto border border-line bg-ink px-3 py-2 font-mono text-[13.5px] leading-relaxed text-paper">{MCP_SNIPPET}</pre>
          </div>
          <div>
            <div className="mb-1 font-mono text-[13px] text-fog">Claude Desktop (Settings → Developer → Edit Config):</div>
            <pre className="overflow-x-auto border border-line bg-ink px-3 py-2 font-mono text-[13.5px] leading-relaxed text-paper">{MCP_DESKTOP}</pre>
          </div>
          <div>
            <div className="mb-1 font-mono text-[13px] text-fog">ChatGPT / Codex (MCP is an open standard — ~/.codex/config.toml):</div>
            <pre className="overflow-x-auto border border-line bg-ink px-3 py-2 font-mono text-[13.5px] leading-relaxed text-paper">{`[mcp_servers.overwrite]
command = "bash"
args = ["-lc", "cd /path/to/overwrite && python3 -m agent.mcp_server"]`}</pre>
          </div>
          <div className="font-serif text-[14px] italic leading-snug text-fog">
      ...
      {open === "terminal" && (
        <div className="space-y-3 border-t-2 border-line px-4 py-3">
          {steps.map((s, i) => (
            <div key={i}>
              <div className="mb-1 font-mono text-[13px] text-fog">
                {i + 1}. {s.title}
              </div>
              <pre className="overflow-x-auto border border-line bg-ink px-3 py-2 font-mono text-[13.5px] leading-relaxed text-paper">
                {s.cmd}
              </pre>
            </div>
          ))}
