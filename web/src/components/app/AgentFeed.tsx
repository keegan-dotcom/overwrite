  ...
  return (
    <div className="border-2 border-line bg-pane">
      ...
      <div className="border-b-2 border-line px-4 py-2.5">
        <div className="mb-1 font-mono text-[10px] uppercase tracking-[0.14em] text-fog">
        ...
        {positions.length === 0 ? (
          ...
          <div className="space-y-1">
            {positions.map((p) => {
              ...
              return (
                <div key={p.id} className="flex items-center justify-between font-mono text-[12px]">
      ...
      {suggestion && (
        <div className="border-b-2 border-amber bg-ink px-4 py-3">
          ...
          <div className="mt-1 font-serif text-[13.5px] font-bold leading-snug text-paper">
            {suggestion.title}
          </div>
          <div className="mt-0.5 font-serif text-[12.5px] leading-snug text-paper/80">
            {suggestion.detail}
          </div>
          <div className="mt-2 flex gap-2">
            <button
              onClick={onAccept}
              className="border-2 border-paper bg-accent px-3 py-1 font-mono text-[11px] font-bold uppercase text-ink shadow-hardsm transition-transform hover:-translate-x-px hover:-translate-y-px"
            >
              Accept
            </button>
            <button
              onClick={onDismiss}
              className="border-2 border-line px-3 py-1 font-mono text-[11px] uppercase text-fog transition-colors hover:border-fog"
            >
              Keep as is
            </button>
          </div>
      ...
      {/* feed */}
      <div className="max-h-56 space-y-1.5 overflow-y-auto px-4 py-2.5 font-mono text-[11.5px] leading-snug">
        {feed.length === 0 && (
          <div className="font-serif text-[12.5px] italic text-fog">
        ...
        {feed.map((e, i) => (
          <div key={i} className="flex gap-2">
            <span className="shrink-0 text-fog/60">{e.ts}</span>
